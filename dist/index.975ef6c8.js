...
searchField.addEventListener("input", (0, _lodashDebounceDefault.default)(onInput, DEBOUNCE_DELAY));
function onInput(e) {
    const countryName = e.target.value.trim();
    (0, _fetchCountries.fetchCountries)(countryName).then((countries)=>{
        clearInfo();
        if (countries.length > 10) return (0, _notiflixDefault.default).Notify.info(`Too many matches found. Please enter a more specific name.`, {
            position: "center-top"
        });
    ...
    }).catch(()=>(0, _notiflixDefault.default).Notify.failure(`Oops, there is no country with that name`, {
            position: "center-top"
        }));
}
function renderCountryInfo(countries) {
    const markup = countries.map((country)=>{
        return `<div class="country-info__item"><h2 class="country-info__title"> <img class="country-info__img" src="${country.flags.svg}"alt="${country.name.official}" width="30" height="30" /> ${country.name.official}</h2><p class="country-info__data"><span class="country-info__subtitle">Capital: </span>${country.capital}</p> <p class="country-info__data"><span class="country-info__subtitle">Population: </span>${country.population}</p><p class="country-info__data"><span class="country-info__subtitle">Languages: </span>${Object.values(country.languages)}</p></div>`;
    }).join("");
